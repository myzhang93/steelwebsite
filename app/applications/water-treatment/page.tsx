...
export default function WaterTreatmentPage() {
  let markdownContent = ""
  try {
    const { content } = getMarkdownByPath('app/customized-parts/Water Treatment.md')
    markdownContent = content
  } catch (error) {
    markdownContent = ""
  }
  
  const config = pageConfigs['applications/water-treatment']
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-40">
              <div className="bg-white border border-gray-200 rounded-lg px-6 pt-4 pb-4 shadow-sm">
                <SidebarQuoteForm source="Water Treatment" />
              </div>
            </div>
          </div>
        </div>
