...
export default function BlogApplicationsPage() {
  let markdownContent = ""
  try {
    const { content } = getMarkdownByPath('app/blog/Applications.md')
    markdownContent = content
  } catch (error) {
    markdownContent = ""
  }
  
  const config = pageConfigs['blog/applications']
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-40">
              <div className="bg-white border border-gray-200 rounded-lg px-6 pt-4 pb-4 shadow-sm">
                <SidebarQuoteForm source="Blog - Applications" />
              </div>
            </div>
          </div>
        </div>
