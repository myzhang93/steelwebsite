...
export default function BlogTrendsPage() {
  let markdownContent = ""
  try {
    const { content } = getMarkdownByPath('app/blog/Trends.md')
    markdownContent = content
  } catch (error) {
    markdownContent = ""
  }
  
  const config = pageConfigs['blog/trends']
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-40">
              <div className="bg-white border border-gray-200 rounded-lg px-6 pt-4 pb-4 shadow-sm">
                <SidebarQuoteForm source="Blog - Trends" />
              </div>
            </div>
          </div>
        </div>
