...
export default function KnowledgeBaseGradesPage() {
  let markdownContent = ""
  try {
    const { content } = getMarkdownByPath('app/Knowledge Base/Grades.md')
    markdownContent = content
  } catch (error) {
    markdownContent = ""
  }
  
  const config = pageConfigs['knowledge-base/grades']
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-40">
              <div className="bg-white border border-gray-200 rounded-lg px-6 pt-4 pb-4 shadow-sm">
                <SidebarQuoteForm source="Knowledge Base - Grades" />
              </div>
            </div>
          </div>
        </div>
