...
export default function SurfaceFinishNo4Page() {
  let markdownContent = ""
  try {
    const { content } = getMarkdownByPath('app/surface finish/no.4.md')
    markdownContent = content
  } catch (error) {
    markdownContent = ""
  }
  
  const config = pageConfigs['surface-finish/no4']
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-40">
              <div className="bg-white border border-gray-200 rounded-lg px-6 pt-4 pb-4 shadow-sm">
                <SidebarQuoteForm source="Surface Finish No.4" />
              </div>
            </div>
          </div>
        </div>
